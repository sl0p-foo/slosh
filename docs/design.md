...
Two decisions do most of the work; the rest follow from them.
...
(`ruff.toml`), imports sorted and then laid out. `make fmt` applies both,
...
system headers, a grouping somebody wrote by hand) while Python import order
