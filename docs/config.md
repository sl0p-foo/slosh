...
| behaviour | `focus_follows_mouse` `ctrl_d_exits` `scroll_lines` `scrollback` `scrollback_bytes` `toast_ms` `splash_ms` `hover_delay_ms` `double_click_ms` `word_separators` `anim_ms` `modal_scrim` `dim_unfocused` `float_shadow` `keep_dead` `in_band_shaders` `shell` `editor` `shader_dir` |
...
`dead`, `suspended`, `bell`, `scrolled`, `floating`, `unfocused`. Exactly one wins,
the first that matches, because two reasons to be grey compound into one muddy
grey that reads as neither. Naming a state replaces its default outright, including
