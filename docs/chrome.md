...
You have already seen one: the default bell flash (a blink, then a breathe on
...
through, which makes this the only kind of pass that can still colour it. That
is the state where a colour is worth the most.
