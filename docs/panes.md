...
- **[`compact true`](config.md#compact)** trades the gaps for shared 1-cell
  divider lines. Same mouse, tmux-style density: dividers drag, crossings
...
search.) `C-a F` opens a **new floating shell**, the throwaway terminal, over
whatever you are doing, in the focused pane's directory; `exit` closes it
