...
**A sidebar's tab numbers sit at the far end of the row**, `api` on the left
and `3` hard against the right edge, rather than as the `3:api` the top strip
writes. `tab_bar_index "prefix"` puts them back:
...
Every name starts in the same column and the numbers line up in one of their
own, which is worth a column of a sidebar and is not available to the top strip
at all — a tab there is exactly as wide as its label, so it has no far end
