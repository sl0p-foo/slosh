...
- **M10** — floating panes (D22): the model and the guards, then the hands —
  ...
  guide — plus `new-float` and the key decisions ✅ (**M10c pending**: floats
  in layout files and `dump-layout`, placement over the API, and the
  kitty-graphics occlusion rule)
