...
void *sl_dlopen(const char *path, int flags) {
  ...
  if (!h)
    snprintf(g_dlerr, sizeof g_dlerr, "LoadLibrary failed (%lu)",
             (unsigned long)GetLastError());
  return (void *)h;
}
void *sl_dlsym(void *lib, const char *sym) {
  FARPROC p = GetProcAddress((HMODULE)lib, sym);
  if (!p) snprintf(g_dlerr, sizeof g_dlerr, "no symbol %s", sym);
  return (void *)(uintptr_t)p;
}
int sl_dlclose(void *lib) { return FreeLibrary((HMODULE)lib) ? 0 : -1; }
const char *sl_dlerror(void) { return g_dlerr[0] ? g_dlerr : NULL; }

/* ---- glob ---------------------------------------------------------------- */

/* Only what shader.c asks for: one directory, one wildcard in the last
 * component. FindFirstFile does the matching. */
int sl_glob(const char *pat, int flags, int (*errfn)(const char *, int),
            glob_t *g) {
  (void)flags;
  (void)errfn;
  if (!g) return GLOB_NOMATCH;
  memset(g, 0, sizeof *g);

  char dir[PATH_MAX];
  snprintf(dir, sizeof dir, "%s", pat);
