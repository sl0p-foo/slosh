...
static size_t parse_one(input_parser_t *p, input_cb_t cb, void *ud) {
  ...
  if (p->in_paste) {
    ...
    for (size_t i = 0; i + sizeof end - 1 <= n; i++) {
      if (memcmp(b + i, end, sizeof end - 1) == 0) {
        input_event_t ev = {
            .kind = EV_PASTE, .paste = (const char *)b, .paste_len = i};
        cb(&ev, ud);
        p->in_paste = false;
  ...
  if (c == 0x1b) {
    ...
    if (b[1] == '[') {
      ...
      if (plen >= 1 && params[0] == '<' && (final == 'M' || final == 'm')) {
        ...
        if (base >= 64 && base <= 67) {
          ev.button =
              (uint8_t)(MBTN_FOUR + (base - 64)); /* wheel up/down/l/r */
  ...
  /* --- C0 controls ----------------------------------------------------- */
  if (c == '\r') {
    emit_key(cb, ud, GHOSTTY_KEY_ENTER, 0, KEY_PRESS, "\r", 1, 13);
    return 1;
  }
  if (c == '\n') {
    emit_key(cb, ud, GHOSTTY_KEY_ENTER, 0, KEY_PRESS, "\r", 1, 13);
  ...
  if (c == '\t') {
    emit_key(cb, ud, GHOSTTY_KEY_TAB, 0, KEY_PRESS, "\t", 1, 9);
    return 1;
  }
  if (c == 0x7f) {
    emit_key(cb, ud, GHOSTTY_KEY_BACKSPACE, 0, KEY_PRESS, NULL, 0, 127);
    return 1;
  }
  if (c == 0x08) {
    emit_key(cb, ud, GHOSTTY_KEY_BACKSPACE, 0, KEY_PRESS, NULL, 0, 8);
    return 1;
  }
  if (c == 0x00) {
    emit_key(cb, ud, GHOSTTY_KEY_SPACE, MOD_CTRL, KEY_PRESS, NULL, 0, ' ');
    return 1;
  }
  if (c < 0x20) {
    int key;
...
void input_event_describe(const input_event_t *ev, char *buf, size_t cap) {
  ...
  case EV_MOUSE: {
    const char *act = ev->maction == MOUSE_PRESS     ? "press"
                      : ev->maction == MOUSE_RELEASE ? "release"
                                                     : "motion";
  ...
  case EV_FOCUS:
    ...
    break;
  default: snprintf(buf, cap, "none"); break;
  }
}
