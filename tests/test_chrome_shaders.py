...
def test_a_chrome_pass_colours_the_frame_and_nothing_else():
    ...
    with Session(SH, cols=60, rows=12, config=conf) as s:
        ...
        check(
            "the frame takes the chrome colour",
            frame_fg(snap, p) == "#ff0000",
            str(frame_fg(snap, p)),
        )
        check(
            "all four sides of it, not just the top",
            fg_at(snap, p["x"], p["y"] + 3) == "#ff0000"
            and fg_at(snap, p["x"] + p["w"] - 1, p["y"] + 3) == "#ff0000"
            and fg_at(snap, p["x"] + 5, p["y"] + p["h"] - 1) == "#ff0000",
            snap.screen(),
        )
        check(
            "and the pane's own output is untouched",
...
def test_a_content_pass_still_leaves_the_frame_alone():
    ...
    with Session(SH, cols=60, rows=12, config=conf) as s:
        ...
        p, snap = s.pane(), s.snapshot()
        check(
            "the contents are tinted",
            content_fg(snap, p) == "#ff0000",
            str(content_fg(snap, p)),
        )
        check("the frame is not", frame_fg(snap, p) == FRAME, str(frame_fg(snap, p)))
...
def test_positions_are_the_whole_frames():
    ...
    with Session(SH, cols=60, rows=12, config=conf) as s:
        ...
        p, snap = s.pane(), s.snapshot()
        check(
            "the top row is the row the expression selected",
            frame_fg(snap, p) == "#00ff00",
            str(frame_fg(snap, p)),
        )
        check(
            "the sides and the bottom are not",
            fg_at(snap, p["x"], p["y"] + 3) == FRAME
            and fg_at(snap, p["x"] + 5, p["y"] + p["h"] - 1) == FRAME,
            snap.screen(),
        )
...
def test_a_state_can_carry_a_chrome_chain():
    ...
    with Session(SH, cols=90, rows=14, config=conf) as s:
        ...
        check(
            "the pane you are not in wears it",
            frame_fg(snap, other) == "#ff0000",
            str(frame_fg(snap, other)),
        )
        check(
            "the one you are in does not",
            frame_fg(snap, focused) != "#ff0000",
            str(frame_fg(snap, focused)),
        )
        ...
        snap = s.snapshot()
        check(
            "and it follows focus rather than sticking to a pane",
            frame_fg(snap, focused) == "#ff0000" and frame_fg(snap, other) != "#ff0000",
...
def test_a_state_block_still_replaces_the_whole_state():
    ...
    with Session(SH, cols=90, rows=14, config=conf) as s:
        ...
        check(
            "the unfocused pane's contents are left alone",
...
def test_a_reload_puts_a_chrome_chain_on_and_takes_it_off_again():
    ...
    with Session(SH, cols=60, rows=12, config=path.name) as s:
        s.until_text("BLOCK")
        check(
            "the frame starts in its own colour",
        ...
        check(
            "saving a chrome pass colours the frame",
        ...
        reload_with(s, "")
        check(
            "and taking it out of the file takes it off the frame",
            frame_fg(s.snapshot(), s.pane()) == FRAME,
            str(frame_fg(s.snapshot(), s.pane())),
        )
    os.unlink(path.name)
...
def test_a_pass_can_be_told_to_leave_the_background_alone():
    ...
    with Session(SH, cols=60, rows=12, config=both) as s:
        ...
        p, snap = s.pane(), s.snapshot()
        check(
            "without a channel the background is painted too",
    ...
    with Session(SH, cols=60, rows=12, config=fg_only) as s:
        ...
        p, snap = s.pane(), s.snapshot()
        check(
            "channel fg still recolours the border glyphs",
            frame_fg(snap, p) not in (FRAME, None),
            str(frame_fg(snap, p)),
        )
        check(
            "and the frame keeps the terminal's own background",
            bg_at(snap, p["x"] + 5, p["y"]) is None,
            str(bg_at(snap, p["x"] + 5, p["y"])),
        )
        check(
            "the contents are still untouched",
            content_fg(snap, p) == GREEN,
            str(content_fg(snap, p)),
        )
...
def test_an_unknown_channel_is_refused_not_guessed():
    ...
    with Session(SH, cols=60, rows=12, config=conf) as s:
        ...
        p, snap = s.pane(), s.snapshot()
        check(
            "the session runs and nothing was shaded",
            s.alive() and frame_fg(snap, p) == FRAME and content_fg(snap, p) == GREEN,
...
def test_a_bell_can_flash_the_border():
    ...
    with Session(SH, cols=90, rows=14, config=conf) as s:
        ...
        check(
            "nothing flashes before anything rings",
        ...
        check(
            "the pane that rang flashes",
            frame_fg(s.snapshot(), other) == "#ff0000",
            str(frame_fg(s.snapshot(), other)),
        )
        check(
            "and asks for the frames to animate it",
            s.deadline() == 20,
            str(s.deadline()),
        )
        check(
            "the pane you are in does not",
        ...
        time.sleep(0.3)
        check(
            "the flash ends by itself, without the bell being answered",
            frame_fg(s.snapshot(), other) == DIM_FRAME,
            str(frame_fg(s.snapshot(), other)),
        )
...
def test_the_bell_state_ends_when_you_look_at_the_pane():
    ...
    with Session(SH, cols=90, rows=14, config=conf) as s:
        ...
        ring(s, other["id"], mine["id"])
        check(
            "it holds while nobody has looked",
            frame_fg(s.snapshot(), other) == "#ff0000",
            str(frame_fg(s.snapshot(), other)),
        )
...
def test_a_rung_pane_is_rung_rather_than_merely_unfocused():
    ...
    with Session(SH, cols=90, rows=14, config=conf) as s:
        ...
        other = [p for p in s.panes() if not p["focused"]][0]
        check(
            "an unfocused pane wears the unfocused chain",
        ...
        ring(s, other["id"], mine["id"])
        check(
            "and the bell chain replaces it once it rings",
            frame_fg(s.snapshot(), other) == "#ff0000",
            str(frame_fg(s.snapshot(), other)),
        )
...
def test_a_collapsed_pane_is_chrome_all_the_way_through():
    ...
    with Session(SH, cols=60, rows=24, config=conf) as s:
        ...
        snap = s.snapshot()
        check(
            "a collapsed row takes the chrome colour",
...
def test_an_unknown_where_is_refused_not_guessed():
    ...
    with Session(SH, cols=60, rows=12, config=conf) as s:
        ...
        check(
            "and neither the frame nor the contents were shaded",
...
def test_an_animated_chrome_pass_asks_for_a_frame_clock():
    ...
    with Session(SH, cols=60, rows=12, config=conf) as s:
        ...
        s.snapshot()
        check(
            "the session wants repainting on the clock it was given",
            s.deadline() == 40,
            str(s.deadline()),
        )
