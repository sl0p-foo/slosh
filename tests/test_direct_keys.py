...
def test_it_fires_with_no_leader():
    with Session(SH, cols=80, rows=14, config=DIRECT) as s:
        ...
        s.settle()
        check(
            "the chord split the pane on its own", len(s.panes()) == 2, str(s.panes())
        )
...
def test_what_it_fires_on_the_program_never_sees():
    ...
    with Session(SH, cols=80, rows=14, config=DIRECT) as s:
        ...
        s.settle(40)
        check(
            "the pane was not sent the keystroke",
            "1;7C" not in s.snapshot().screen(),
            s.snapshot().screen()[:300],
        )
...
def test_everything_else_still_reaches_the_program():
    with Session(SH, cols=80, rows=14, config=DIRECT) as s:
        ...
        check(
            "an unbound chord goes straight through",
            "1;7D" in snap.screen(),
            snap.screen()[:300],
        )
...
def test_it_also_works_after_the_leader():
    ...
    with Session(SH, cols=80, rows=14, config=DIRECT) as s:
        ...
        s.settle()
        check(
            "the leader then the chord does the same thing",
            len(s.panes()) == 2,
            str(s.panes()),
        )
...
def test_the_leader_itself_cannot_be_stolen():
    ...
    with Session(SH, cols=80, rows=14, config=conf) as s:
        ...
        s.settle()
        check(
            "the prefix is still the prefix",
...
def test_the_cheatsheet_lists_them_apart():
    with Session(SH, cols=96, rows=40, config=DIRECT) as s:
        ...
        check("under a heading of their own", "without the leader" in screen, screen)
...
def test_none_unbinds_a_direct_binding():
    ...
    with Session(SH, cols=80, rows=14, config=conf) as s:
        ...
        check(
            "and the chord goes back to the program",
            "1;7C" in snap.screen(),
            snap.screen()[:300],
        )
