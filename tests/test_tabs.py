...
def test_dragging_a_tab_reorders_the_strip():
    with Session(SH, cols=80, rows=12) as s:
        ...
        check("the order to start", order(s) == ["", "one", "two", "three"],
              str(order(s)))
        ...
        check("the dragged tab lands where it was dropped",
              order(s) == ["", "three", "one", "two"], str(order(s)))
...
def test_dragging_past_the_last_tab_puts_it_last():
    with Session(SH, cols=80, rows=12) as s:
        ...
        check("dropping past the end means last",
              order(s) == ["one", "two", ""], str(order(s)))
...
def test_dragging_off_the_strip_moves_nothing():
    ...
    with Session(SH, cols=80, rows=12) as s:
        ...
        release(s, 20, 6)
        check("the order is untouched", order(s) == ["", "one", "two"],
              str(order(s)))
...
def test_move_tab_over_the_api_keeps_you_where_you_are():
    ...
    with Session(SH, cols=80, rows=12) as s:
        ...
        check("the strip is reordered", order(s) == ["c", "", "a", "b"],
              str(order(s)))
        check("and you are still in the tab you were in", active(s) == "a",
              active(s))

        check("an unknown tab is refused",
              s.api("move-tab", id=999, index=1).get("ok") is False)
        check("and so is an index off the end",
...
def test_double_clicking_a_tab_renames_it():
    ...
    with Session(SH, cols=70, rows=12) as s:
        ...
        snap = s.snapshot()
        check("the editor opens in the tab's own cell",
              "\u2588" in snap.line(1), repr(snap.line(1)))

        s.send("logs")
        check("typing goes into it", "logs\u2588" in s.snapshot().line(1),
              repr(s.snapshot().line(1)))

        s.send(r"\r")
        check("enter keeps it", s.tabs()[1]["name"] == "logs",
              str(s.tabs()))
        check("and the strip shows it", "2:logs" in s.snapshot().line(1),
              repr(s.snapshot().line(1)))
...
def test_escape_abandons_and_clicking_away_keeps():
    with Session(SH, cols=70, rows=12) as s:
        ...
        s.send("throwaway")
        s.send(r"\x1b")
        check("escape leaves the name alone", s.tabs()[1]["name"] == "",
              str(s.tabs()))
        ...
        for _ in range(len("kept")):
            s.send(r"\x7f")
        s.send(r"\r")
        check("an empty name clears it", s.tabs()[1]["name"] == "",
              str(s.tabs()))
...
def test_a_tab_and_a_pane_with_the_same_id_are_not_the_same_thing():
    ...
    with Session(titled, cols=70, rows=12) as s:
        s.settle()
        s.key("c")
        s.until(lambda sn: any(h["action"].startswith("panetitle:")
                               for h in sn.hits))
        snap = s.snapshot()
        pane_id = s.focused()["id"]  # the new tab's pane, not tab 1's
        tab_id = s.tabs()[1]["id"]
        check("the ids really do collide", pane_id == tab_id,
              f"pane {pane_id} tab {tab_id}")

        title = [h for h in snap.hits
                 if h["action"] == f"panetitle:{pane_id}"][0]
        strip = strip_hits(snap)[f"tab:{tab_id}"]

        s.click(strip["x"] + 1, strip["y"])
        s.click(title["x"] + 1, title["y"])
        check("one click on each opens no editor",
              "\u2588" not in s.snapshot().screen(), s.snapshot().screen())

        dbl(s, title)
        check("but two on the pane still open its own",
              "\u2588" in s.snapshot().screen(), s.snapshot().screen())
