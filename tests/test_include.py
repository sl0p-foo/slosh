...
def test_an_included_theme_applies():
    root = tree(
        {"themes/green.kdl": GREEN, "config.kdl": 'include "themes/green.kdl"\n'}
    )
    with Session(SH, cols=60, rows=14, config=root + "/config.kdl") as s:
        s.settle()
        check(
            "the frame wears the included colour",
            frame_fg(s) == "#00ff00",
            str(frame_fg(s)),
        )
...
def test_the_file_doing_the_including_wins():
    ...
    with Session(SH, cols=60, rows=14, config=root + "/config.kdl") as s:
        s.settle()
        check(
            "the outer file's colour is the one on screen",
            frame_fg(s) == "#0000ff",
            str(frame_fg(s)),
        )
    ...
    root = tree(
        {"themes/green.kdl": GREEN, "config.kdl": BLUE + 'include "themes/green.kdl"\n'}
    )
    with Session(SH, cols=60, rows=14, config=root + "/config.kdl") as s:
        s.settle()
        check(
            "even with the include written last",
            frame_fg(s) == "#0000ff",
            str(frame_fg(s)),
        )
...
def test_later_includes_win_over_earlier_ones():
    ...
    with Session(SH, cols=60, rows=14, config=root + "/config.kdl") as s:
        s.settle()
        check(
            "the last one applied is the one that shows",
            frame_fg(s) == "#0000ff",
            str(frame_fg(s)),
        )
...
def test_a_path_is_relative_to_the_file_that_wrote_it():
    ...
    root = tree(
        {
            "themes/base/core.kdl": 'theme { frame_focus "#ff00ff" }\n',
    ...
    with Session(SH, cols=60, rows=20, config=root + "/config.kdl") as s:
        s.settle()
        check("the nested include resolved", frame_fg(s) == "#ff00ff", str(frame_fg(s)))
...
def test_a_leading_tilde_is_a_home_directory():
    ...
    with Session(
        SH, cols=60, rows=14, config=root + "/config.kdl", env={"HOME": home}
    ) as s:
        s.settle()
        check(
...
def test_anything_can_be_included_not_only_a_theme():
    ...
    with Session(SH, cols=90, rows=14, config=root + "/config.kdl") as s:
        ...
        s.settle()
        check(
            "a binding from an included file works", len(s.panes()) == 2, str(s.panes())
        )
...
def test_a_missing_include_costs_a_line_and_no_more():
    ...
    with Session(SH, cols=60, rows=14, config=root + "/config.kdl") as s:
        ...
        check(
            "and the rest of the file applied anyway", s.pane()["x"] == 4, str(s.pane())
        )

        reply = s.api("reload")
        check("the reload succeeds", reply.get("ok"), str(reply))
        check(
            "...and says which line it could not honour",
            "nope.kdl" in reply.get("warning", ""),
            str(reply),
        )
        check(
            "the session says it out loud too",
...
def test_a_cycle_stops_rather_than_spinning():
    ...
    with Session(SH, cols=60, rows=14, config=root + "/a.kdl") as s:
        ...
        check(
            "and the file that included itself still applied",
            frame_fg(s) == "#00ff00",
            str(frame_fg(s)),
        )
...
def test_an_include_is_picked_up_by_a_reload():
    ...
    with Session(SH, cols=60, rows=14, config=root + "/config.kdl") as s:
        ...
        s.settle(30)
        check(
            "and blue after the include line changed",
            frame_fg(s) == "#0000ff",
            str(frame_fg(s)),
        )
