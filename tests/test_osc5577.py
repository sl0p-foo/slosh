...
def test_status_and_buttons():
    with Session(emit(osc("1;status;building 3/7"),
                      osc("1;buttons;approve:Approve;cancel:Cancel")),
                 cols=62, rows=8) as s:
        s.settle()
        snap, p = s.snapshot(), s.pane()
        bottom = snap.line(p["y"] + p["h"] - 1)
        check("status text is drawn in the frame", "building 3/7" in bottom,
              repr(bottom))
        check("buttons are drawn", "[Approve]" in bottom and "[Cancel]" in bottom,
              repr(bottom))
        check("buttons are anchored right of the status",
              bottom.index("building") < bottom.index("[Approve]"), repr(bottom))
        check("the frame is intact around it",
              bottom.startswith("  ╰") and bottom.rstrip().endswith("╯"),
              repr(bottom))

        # nothing of the sequence itself may reach the screen
        check("the escape sequence is not rendered", "5577" not in snap.screen(),
              repr(snap.screen()[:120]))


def test_click_reports():
    with Session(emit(osc("1;buttons;approve:Approve;cancel:Cancel")),
                 cols=62, rows=8) as s:
        s.settle()
        snap, p = s.snapshot(), s.pane()
        ...
        check("the click target is where the glyph is",
              action == f"btn:{p['id']}:approve", str(action))
        check("outside the button is not the button",
        ...
        check("clicking sends a click report to the pane",
              "]5577;1;click;approve" in out, repr(out[:120]))
...
def test_replace_and_clear():
    with Session(shell(), cols=50, rows=8) as s:
        ...
        s.settle()
        check("status replaces, it does not append",
              "second" in bottom() and "first" not in bottom(), repr(bottom()))
        check("buttons are untouched by a status update", "[A]" in bottom(),
              repr(bottom()))

        s.raw(emit_cmd("1;clear"))
        s.settle()
        check("clear removes both",
              "second" not in bottom() and "[A]" not in bottom(), repr(bottom()))


def test_escaping():
    with Session(emit(osc("1;status;a;b;c"),
                      osc("1;buttons;go:one%3Btwo;colon:a%3Ab;pct:100%25")),
                 cols=70, rows=8) as s:
        s.settle()
        p = s.pane()
        bottom = s.snapshot().line(p["y"] + p["h"] - 1)
        check("status keeps its semicolons (everything after status; is text)",
              "a;b;c" in bottom, repr(bottom))
        check("a %3B in a label becomes a semicolon", "[one;two]" in bottom,
              repr(bottom))
...
def test_hostile_input():
    ...
    with Session(emit(osc("2;status;from the future"),
        ...
        snap = s.snapshot()
        bottom = snap.line(p["y"] + p["h"] - 1)
        check("a future version is ignored entirely",
              "from the future" not in bottom, repr(bottom))
        ...
        check("an empty label is dropped", snap.hit_at(p["x"] + p["w"] - 3,
                                                       p["y"] + p["h"] - 1) is None
              or "ok" not in str(snap.hits), str(snap.hits))
        check("an over-long id is dropped", "[TooLong]" not in bottom, repr(bottom))
        check("valid sequences after invalid ones still work",
              "legit" in bottom, repr(bottom))

    # in-band purpose: allowed when free, refused when declared
    with Session(emit(osc("1;purpose;agent:main")), cols=50, rows=8) as s:
        s.settle()
        p = s.pane()
        check("a pane may label itself when nothing is declared",
              p["purpose"] == "agent:main" and not p["purpose_declared"], str(p))
...
def test_split_across_reads():
    ...
    with Session(shell(), cols=60, rows=8) as s:
        ...
        s.settle()
        bottom = s.snapshot().line(p["y"] + p["h"] - 1)
        check("a sequence split across reads is still understood",
              "in pieces" in bottom, repr(bottom))
...
def test_pi_extension_compat():
    """The exact byte patterns sl0ppi's pi extensions emit (answer-picker).
        ...
        escLabel: % -> %25, ; -> %3B, : -> %3A
    """
    with Session(emit(osc("1;buttons;ans-continue:continue;ans-yes:yes"),
                      osc("1;status;alt+N · ready")),
                 cols=70, rows=8) as s:
        s.settle()
        p, snap = s.pane(), s.snapshot()
        bottom = snap.line(p["y"] + p["h"] - 1)
        check("answer-picker's buttons render",
              "[continue]" in bottom and "[yes]" in bottom, repr(bottom))
        check("answer-picker's status renders", "alt+N · ready" in bottom,
              repr(bottom))
        ...
        check("the click report matches the extension's CLICK_RE",
              "^[]5577;1;click;ans-continue^[\\" in out, repr(out[:140]))
    ...
    with Session(shell(), cols=70, rows=8) as s:
        ...
        s.settle()
        check("status and buttons together", "[yes]" in bottom() and "ready" in bottom(),
              repr(bottom()))

        s.raw(emit_cmd("1;buttons"))  # hideButtons(): verb with no payload
        s.settle()
        check("`buttons` with no payload drops only the buttons",
              "[yes]" not in bottom() and "ready" in bottom(), repr(bottom()))
