...
def test_a_purpose_can_be_typed_where_it_will_live():
    with Session(SH) as s:
        s.key("P")
        snap = s.snapshot()
        check("the editor announces which label it is editing",
              "purpose" in snap.screen(), snap.screen())

        s.send("service:web")
        check("what is typed shows in the title cell",
              "purpose service:web" in s.snapshot().screen(), s.snapshot().screen())

        s.send("\\r")
        check("enter keeps it", s.pane()["purpose"] == "service:web", str(s.pane()))
        check("and it counts as declared, because an operator set it",
              s.pane()["purpose_declared"] is True, str(s.pane()))
        check("the editor is gone",
...
def test_escape_abandons_it():
    with Session(SH) as s:
        ...
        s.send("half-typed")
        s.send("\\033")
        check("escape leaves the purpose alone", s.pane()["purpose"] == "",
              str(s.pane()))
...
def test_it_seeds_with_what_is_there():
    ...
    with Session(SH) as s:
        ...
        s.key("P")
        check("the editor opens on the purpose it has",
              "purpose agent:main" in s.snapshot().screen(), s.snapshot().screen())
        s.send(":2\\r")
        check("so an edit is an edit", s.pane()["purpose"] == "agent:main:2",
              str(s.pane()))
...
def test_a_typed_purpose_outranks_the_program_in_the_pane():
    ...
    with Session(shell(), cols=60, rows=8) as s:
        ...
        s.settle()
        check("a pane cannot relabel a purpose an operator declared",
...
def test_clearing_it_hands_the_label_back_to_the_program():
    ...
    with Session(shell(), cols=60, rows=8) as s:
        ...
        s.settle()
        check("so the pane can speak for itself again",
              s.pane()["purpose"] == "mine-now", str(s.pane()))
...
def test_the_action_is_bindable_and_in_the_palette():
    with Session(SH) as s:
        ...
        s.send("purpose")
        snap = s.snapshot()
        check("the palette offers it without it having to be bound",
              "tag it with a purpose" in snap.screen(), snap.screen())
        s.send("\\r")
        check("and choosing it opens the editor",
              "purpose" in s.snapshot().screen(), s.snapshot().screen())
...
def test_id_zero_is_the_focused_pane_here_too():
    ...
    with Session(SH) as s:
        ...
        check("and it tagged the focused pane",
              focused["purpose"] == "service:web", str(focused))
        others = [p for p in s.panes() if not p["focused"]]
        check("and nothing else", all(p["purpose"] == "" for p in others),
              str(others))

        t = s.api("set-purpose", target="tab", id=0, purpose="notes")
        check("a tab takes 0 as the one you are looking at", t["ok"] is True,
              str(t))
        here = [x for x in s.tabs() if x["active"]][0]
        check("and that is the tab it tagged", here["purpose"] == "notes",
              str(here))
...
def test_a_dumped_layout_carries_what_was_tagged_by_hand():
    ...
    with Session(SH) as s:
        ...
        check("both tags are in the file", 'purpose="agent:main"' in kdl and
              'purpose="shell:scratch"' in kdl, kdl)
