...
def check_at_rest(s, label):
    ...
    for p in s.panes():
        ...
        check(
            f"{label}: pane {p['id']} is at rest", ok, f"{fg} (focused={p['focused']})"
        )
...
def test_nothing_is_dimmed_with_the_knob_at_zero():
    with Session(SH, cols=80, rows=14, config=cfg("dim_unfocused 0\n")) as s:
        ...
        snap = s.snapshot()
        check(
            "neither pane is touched",
            content_fg(snap, other) == GREEN and content_fg(snap, focused) == GREEN,
...
def test_dims_everything_but_the_focused_pane():
    with Session(SH, cols=80, rows=14, config=cfg(unfocused_cfg(128))) as s:
        ...
        snap = s.snapshot()
        check(
            "the focused pane keeps its colour",
            content_fg(snap, focused) == GREEN,
            str(content_fg(snap, focused)),
        )
        check(
            "an unfocused pane is dimmed",
            content_fg(snap, other) not in (GREEN, None),
            str(content_fg(snap, other)),
        )
        check(
            "dimmed means darker, not merely different",
...
def test_the_dimming_follows_focus():
    with Session(SH, cols=80, rows=14, config=cfg(unfocused_cfg(128))) as s:
        ...
        check(
            "the newly focused pane is undimmed",
            content_fg(snap, now_focused) == GREEN,
            str(content_fg(snap, now_focused)),
        )
        check(
            "and the one that lost focus is dimmed now",
...
def test_dimming_never_touches_the_chrome():
    with Session(SH, cols=80, rows=14, config=cfg(unfocused_cfg(200))) as s:
        ...
        snap = s.snapshot()
        check(
            "a dimmed pane's frame keeps its full colour",
            frame_fg(snap, other) == "#45454a",
        ...
        check(
            "and the tab strip is untouched",
...
def test_dragging_greys_the_other_panes():
    with Session(SH, cols=90, rows=16) as s:
        ...
        check(
            ...
            content_fg(snap, left) == GREEN,
            str(content_fg(snap, left)),
        )
        check(
            "every other pane goes grey",
        ...
        check(
            "the colour has collapsed towards grey",
            max(ch) - min(ch) < 80,
            f"{grey} spread={max(ch) - min(ch)}",
        )
        check(
            "the two channels that started equal stay equal", ch[0] == ch[2], str(grey)
        )
...
def test_full_strength_is_a_true_grey():
    with Session(
        SH, cols=90, rows=16, config=cfg(drag_cfg("grayscale amount=255"))
    ) as s:
        s.until_text("BLOCK")
        left, right = split(s)
        ...
        check(
            "at 255 every channel agrees exactly",
            grey and grey[1:3] == grey[3:5] == grey[5:7],
            str(grey),
        )
        release(s, right["x"] + 5, right["y"] + 3)
        s.settle(120)
...
def test_the_two_drag_knobs_are_independent():
    ...
    check(
        "with both off a drag leaves every pane alone", both_off == GREEN, str(both_off)
    )
    ...
    check(
        "grayscale alone desaturates without darkening much",
        ch[0] == ch[1] == ch[2] and ch[1] > 0x60,
        str(gray_only),
    )
    ...
    check(
        "dim alone darkens without desaturating",
        ch[0] == 0 and ch[2] == 0 and 0 < ch[1] < 0x60,
        str(dim_only),
    )
...
def test_the_default_drag_look_is_grey_and_recessed():
    ...
    check(
        "a non-dragged pane is pushed well below full brightness",
        max(ch) < 0x80,
        f"{got} max={max(ch):#x}",
    )
    check(
        "and towards grey", max(ch) - min(ch) < 60, f"{got} spread={max(ch) - min(ch)}"
    )
...
def test_non_dragged_panes_get_a_dashed_border():
    with Session(SH, cols=90, rows=16) as s:
        ...
        snap = s.snapshot()
        check(
            "borders are solid when nothing is being dragged",
            DASH_H not in snap.line(right["y"]),
            repr(snap.line(right["y"])),
        )
        ...
        check(
            "the pane in your hand stays solid",
        ...
        for p in s.panes():
            ...
            check(
                f"pane {p['id']} is solid again after the drop",
                DASH_H not in row,
                repr(row),
            )
...
def test_a_press_that_never_moves_dashes_nothing():
    with Session(SH, cols=90, rows=16) as s:
        ...
        press(s, left["x"] + 4, left["y"])
        snap = s.snapshot()
        check("pressing without moving dashes nothing", DASH_H not in snap.screen(), "")
        release(s, left["x"] + 4, left["y"])
        s.settle(120)
...
def test_config_shaders_reach_the_screen():
    ...
    with Session(SH, cols=60, rows=12, config=cfg(conf)) as s:
        ...
        check(
            "a configured ruler marks its column",
            (on or {}).get("bg") == "#ff0000",
            str(on),
        )
        check(
            "and leaves the neighbouring column alone",
            (off or {}).get("bg") != "#ff0000",
            str(off),
        )
...
def test_config_shaders_run_in_written_order():
    ...
    check(
        "the last shader written is the last one applied",
...
def test_an_unknown_shader_is_refused_not_guessed():
    with Session(
        SH, cols=50, rows=10, config=cfg("shaders {\n    bloom amount=255\n}\n")
    ) as s:
        s.until_text("BLOCK")
        check(
            "an unknown shader leaves the session running and unshaded",
...
def test_the_hovered_pane_is_a_different_state_from_the_rest():
    ...
    with Session(SH, cols=120, rows=20, config=cfg(conf)) as s:
        ...
        check(
            ...
            content_fg(snap, b) == "#ff0000",
            str(content_fg(snap, b)),
        )
        check(
            "the other candidates are the plain target state",
            content_fg(snap, c) == "#0000ff",
            str(content_fg(snap, c)),
        )
...
def test_a_suspended_pane_is_a_state():
    ...
    with Session(SH, cols=90, rows=14, config=cfg(conf), layout=l) as s:
        ...
        check(
            "a suspended pane reaches the shader pass at all",
            (run or {}).get("fg") == "#00aaff",
            str(run),
        )
...
def test_scrollback_is_a_state():
    ...
    with Session(noisy, cols=80, rows=14, config=cfg(conf)) as s:
        ...
        p = s.pane()
        check(
            "not scrolled yet, so no state colour",
        ...
        check(
            "looking at the past is a state you can colour",
...
def test_exactly_one_state_wins():
    ...
    with Session(SH, cols=90, rows=14, config=cfg(conf), layout=l) as s:
        ...
        if not susp or susp[0]["focused"]:
            check(
                "a suspended, unfocused pane to test with", bool(susp), str(s.panes())
            )
            return
        ...
        check(
            "suspended outranks unfocused, and they do not mix",
            (run or {}).get("fg") == "#00aaff",
            str(run),
        )
...
def test_an_unknown_state_is_refused():
    ...
    with Session(SH, cols=60, rows=12, config=cfg(conf)) as s:
        s.until_text("BLOCK")
        check(
            "an unknown state name leaves the session running",
            s.alive() and content_fg(s.snapshot(), s.pane()) == GREEN,
            str(content_fg(s.snapshot(), s.pane())),
        )
