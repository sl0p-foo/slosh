...
def run(outdir, docs=None, expect_ok=True):
    ...
    r = subprocess.run([sys.executable, GEN, outdir], capture_output=True,
                       text=True, env=env)
...
def test_it_renders_every_page_in_the_nav():
    ...
    check("docs/nav has sections and pages", sections and len(pages) >= 8,
          f"{sections} / {pages}")

    for md in pages:
        f = os.path.join(out, md[:-3] + ".html")
        check(f"{md} was rendered", os.path.exists(f), f)
    check("the stylesheet came with it",
          os.path.exists(os.path.join(out, "docs.css")))

    body = open(os.path.join(out, "keys.html")).read()
    for md in pages:
        check(f"keys.html links to {md} in its sidebar",
              'href="%s"' % (md[:-3] + ".html") in body, md)
    check("the current page is marked in the sidebar",
          'class="here" href="keys.html"' in body, "no here marker")
...
def test_prev_and_next_chain_the_nav_order():
    ...
    check("the first page has a next and no prev",
          'class="next"' in first and 'class="prev"' not in first, "first")
    check("a middle page has both",
          'class="next"' in middle and 'class="prev"' in middle, "middle")
    check("the last page has a prev and no next",
          'class="prev"' in last and 'class="next"' not in last, "last")
...
def test_a_page_missing_from_the_nav_is_refused():
    ...
    check("and it says which one", "orphan.md" in (r.stderr + r.stdout),
          r.stderr + r.stdout)
...
def test_a_nav_entry_with_no_file_is_refused():
    ...
    check("and it says which one", "ghost.md" in (r.stderr + r.stdout),
          r.stderr + r.stdout)
...
""")
    cases = {
        ...
        "headings get an id and an anchor":
            '<h2 id="a-heading">' in got and 'href="#a-heading"' in got,
        "inline code": "<code>code</code>" in got,
        "bold": "<strong>bold</strong>" in got,
        "italic": "<em>italic</em>" in got,
        "a link to another page points at its html":
            'href="keys.html#defaults"' in got,
        ...
        "and its contents verbatim":
            html.escape('dim amount="(y % 2) * 40"') in got,
...
def test_markup_inside_a_code_span_is_not_markup():
    ...
    check("asterisks in a code span survive",
          html.escape('(x < 10) * 200 ** 2') in got, got[:400])
    check("bold outside it still works", "<strong>bold</strong>" in got, got[:400])
    check("and the span is not a strong tag",
          "<strong>" in got and got.count("<strong>") == 1, got[:400])
