...
def test_status_bar():
    with Session(SH, cols=70, rows=14) as s:
        ...
        x = btn[0]["x"] + 1
        check(
            "the new-tab button is clickable",
            snap.hit_at(x, 1) == "newtab",
            str(snap.hit_at(x, 1)),
        )
        s.click(x, 1)
        s.settle()
    ...
    with Session(SH, cols=70, rows=14) as s:
        ...
        s.settle(60)
        check(
            "a held prefix is shown as a mode",
            "C-a" in s.snapshot().line(1),
            repr(s.snapshot().line(1)),
        )
...
def is_open(s):
    snap = s.snapshot()
    return any(
        h["action"].startswith("find:") or h["action"] == "closefind" for h in snap.hits
    )
...
def test_finder_keyboard():
    with Session(SH, cols=70, rows=16) as s:
        ...
        check(
            ...
            "service:web" in snap.screen() and "agent:main" not in snap.screen(),
            repr(snap.screen()),
        )
        check(
            "the query is shown as you type",
            "web" in snap.screen(),
            repr(snap.screen()),
        )
        ...
        s.send(r"\r")
        s.settle(80)
        check("enter focuses the match", s.focused()["id"] == b, str(s.focused()))
    ...
    with Session(SH, cols=70, rows=16) as s:
        ...
        s.settle(80)
        check(
            "backspace restores the list",
            "agent:main" in s.snapshot().screen(),
...
def test_finder_query_editing():
    with Session(SH, cols=70, rows=16) as s:
        ...
        check(
            ...
            "\u00e9" in s.snapshot().screen(),
            repr(s.snapshot().screen()[:300]),
        )
        s.send(r"\x7f")
        s.settle(60)
        check(
            "and one backspace removes all of it",
...
def test_finder_mouse():
    with Session(SH, cols=70, rows=16) as s:
        ...
        x, y = pos
        check(
            "finder rows sit on top of the hit list",
            snap.hit_at(x, y) == f"find:{b}",
            str(snap.hit_at(x, y)),
        )
        s.click(x, y)
        s.settle(80)
        check(
            "clicking a row focuses that pane", s.focused()["id"] == b, str(s.focused())
        )
...
def test_finder_dismissal_by_mouse():
    ...
    with Session(SH, cols=70, rows=16) as s:
        ...
        check(
            "it has a close button, like any modal",
            len(close) == 1,
            str(s.snapshot().hits),
        )
        if close:
        ...
        check(
            "and does not reach the pane behind it",
            s.focused()["id"] == b,
            str(s.focused()),
        )
...
def test_finder_marks_where_you_are():
    with Session(SH, cols=70, rows=16) as s:
        ...
        check(
            "exactly one row is marked as the current pane",
            len(marked) == 1,
            str(rows(s)),
        )
        if marked:
            check(
                "and it is the pane we are in", marked[0][0] == f"find:{b}", str(marked)
            )
...
def test_finder_over_a_collapsed_layout():
    ...
    with Session(SH, cols=120, rows=32) as s:
        ...
        s.settle(80)
        check(
            "the finder reaches a collapsed pane and expands it",
