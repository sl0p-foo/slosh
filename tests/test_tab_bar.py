...
def test_left_sidebar_reserves_columns():
    with Session(SH, cols=90, rows=20, config=LEFT) as s:
        s.settle(30)
        snap = s.snapshot()
        # gap 1 (the default): the sidebar's first row is row 1, like the strip.
        check(
            "the first tab is a row in the sidebar",
            snap.hit_at(1, 1) == "tab:1",
            str(snap.hit_at(1, 1)),
        )
        check(
...
def test_sidebar_rows_click_like_the_strip():
    with Session(SH, cols=90, rows=20, config=LEFT) as s:
        s.settle(30)
        s.click(1, 2)  # the + row: a new tab, which becomes the active one
        ...
        check(
            ...
            snap.hit_at(1, 2) == "tab:2",
            str(snap.hit_at(1, 2)),
        )
        s.click(1, 1)  # the first tab's row
...
def test_right_sidebar_takes_the_other_edge():
    with Session(SH, cols=90, rows=20, config=RIGHT) as s:
        ...
        check(
            ...
            snap.hit_at(x0 + 1, 1) == "tab:1",
            str(snap.hit_at(x0 + 1, 1)),
        )
        p = s.pane(0)
        check(
...
def test_growing_back_restores_the_sidebar():
    with Session(SH, cols=40, rows=20, config=LEFT) as s:
        s.settle(30)
        s.resize(90, 20)
        s.until(lambda snap: snap.hit_at(1, 1) == "tab:1")
        check(
            "a resize moves the bar without anyone storing state",
            s.snapshot().hit_at(1, 1) == "tab:1",
            str(s.snapshot().hit_at(1, 1)),
        )
        p = s.pane(0)
