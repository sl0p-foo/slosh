...
def test_two_rows_become_two_columns_the_right_way_round():
    with Session(SH, cols=80, rows=24) as s:
        ...
        top, bottom = rects(s)
        check(
            "it starts as two rows",
            top[2] < bottom[2] and top[3] == bottom[3],
            str(rects(s)),
        )
        ...
        check(
            "one turn makes two columns",
            left[2] == right[2] and left[1] < right[1],
            str(rects(s)),
        )
        check(
            "and the pane that was on top is now on the right",
            right[0] == top[0],
            f"top was {top[0]}, right is {right[0]}",
        )
        check(
            "the one that was underneath is on the left",
            left[0] == bottom[0],
            f"bottom was {bottom[0]}, left is {left[0]}",
        )

        turn(s)
        a, b = rects(s)
        check(
            "two turns are two rows again", a[3] == b[3] and a[2] < b[2], str(rects(s))
        )
        check(
            "...with the panes the other way up than they started",
            a[0] == bottom[0] and b[0] == top[0],
            str(rects(s)),
        )
...
def test_four_turns_are_the_identity():
    ...
    with Session(SH, cols=100, rows=30) as s:
        ...
        for i in range(1, 4):
            turn(s)
            seen.append(rects(s))
            check(f"turn {i} changes the arrangement", seen[-1] != start, str(seen[-1]))
        turn(s)
        check(
            "the fourth turn comes back exactly",
            rects(s) == start,
            f"{start} -> {rects(s)}",
        )
...
def test_a_share_of_one_axis_becomes_a_share_of_the_other():
    with Session(SH, cols=100, rows=30) as s:
        ...
        left, right = sorted(rects(s), key=lambda r: r[1])
        check(
            "the columns are uneven", left[3] > right[3] + 8, f"{left[3]} vs {right[3]}"
        )
        ...
        check(
            "the wider pane is now the taller one",
            top[0] == left[0] and top[4] > bottom[4],
            f"{rects(s)} (left was {left[0]})",
        )
        check(
            "and it kept roughly the share it had",
...
def test_focus_stays_on_the_pane_you_were_in():
    with Session(SH, cols=90, rows=24) as s:
        ...
        turn(s, 3)
        check(
            "the pane you were in is the pane you are in",
            s.focused()["id"] == was,
            f"{was} -> {s.focused()['id']}",
        )
...
def test_a_turn_that_would_not_fit_is_refused():
    ...
    with Session(SH, cols=100, rows=30) as s:
        ...
        check(
            "evening it out first lets the same turn through",
            cols[0][2] == cols[1][2] and cols[0][1] < cols[1][1],
            str(rects(s)),
        )
...
def test_one_pane_has_nothing_to_turn():
    with Session(SH, cols=80, rows=24) as s:
        ...
        check(
            "it says so",
            "nothing to turn" in s.snapshot().screen(),
            repr(s.snapshot().screen()[-200:]),
        )
        check(
            "and the pane is untouched", rects(s) == before, f"{before} -> {rects(s)}"
        )
...
def test_it_is_offered_by_name():
    with Session(SH, cols=110, rows=40) as s:
        ...
        s.settle()
        check(
            "the cheatsheet lists it with its key",
...
def test_a_rotated_layout_survives_a_dump():
    ...
    with Session(SH, cols=90, rows=24, layout=lay.name) as s2:
        s2.settle(60)
        check(
            "and comes back in the same arrangement",
