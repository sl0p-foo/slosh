...
def test_text_and_cursor():
    with Session(SH("stty raw -echo; cat"), cols=30, rows=8) as s:
        ...
        p = s.pane()
        snap = s.snapshot()
        check("text lands on the pane's first row",
              snap.pane_line(p, 0).startswith("hello"), repr(snap.pane_line(p, 0)))
        check("cursor follows the text",
              (snap.cursor["x"], snap.cursor["y"]) == (p["content_x"] + 5, p["content_y"]),
              str(snap.cursor))

        s.raw("\\r\\nsecond")
        s.settle()
        snap = s.snapshot()
        check("newline moves to the next row",
              snap.pane_line(p, 1).startswith("second"), repr(snap.pane_line(p, 1)))
        check("cursor is on that row",
              snap.cursor["y"] == p["content_y"] + 1, str(snap.cursor))


def test_styles():
    with Session(SH(r'printf "n\033[1mB\033[31mR\033[0mn"; sleep 5'), cols=30, rows=6) as s:
        s.settle()
        p, snap = s.pane(), s.snapshot()
        cx, cy = p["content_x"], p["content_y"]
        check("plain cell has no style run", snap.style_at(cx, cy) is None,
              str(snap.style_at(cx, cy)))
        b = snap.style_at(cx + 1, cy)
        check("bold is recorded", b and b["attrs"] == ["bold"], str(b))
        r = snap.style_at(cx + 2, cy)
        check("colour is resolved to rgb",
              r and r["fg"] is not None and "bold" in r["attrs"], str(r))
        check("reset ends the run", snap.style_at(cx + 3, cy) is None,
...
def test_wide_and_clusters():
    with Session(SH('printf "[\\346\\227\\245]"; sleep 5'), cols=30, rows=6) as s:
        s.settle()
        p, snap = s.pane(), s.snapshot()
        check("wide char occupies one cell in the dump",
              snap.pane_line(p, 0).startswith("[日]"), repr(snap.pane_line(p, 0)))

    with Session(SH('printf "[\\360\\237\\207\\263\\360\\237\\207\\261]"; sleep 5'),
                 cols=30, rows=6) as s:
        s.settle()
        p, snap = s.pane(), s.snapshot()
        check("a flag is one grapheme cluster, not two",
              snap.pane_line(p, 0).startswith("[🇳🇱]"), repr(snap.pane_line(p, 0)))
...
def test_terminal_replies():
    ...
    with Session(SH('printf "\\033]2;build.sh\\007"; sleep 5'), cols=44, rows=8) as s:
        ...
        check("the title is drawn in the frame",
              "build.sh" in s.snapshot().line(p["y"]), repr(s.snapshot().line(p["y"])))
...
def test_hitlist_contract():
    with Session(SH("sleep 5"), cols=30, rows=6) as s:
        s.settle()
        p, snap = s.pane(), s.snapshot()
        check("the pane body is hit-testable",
              snap.hit_at(p["content_x"], p["content_y"]) == f"pane:{p['id']}",
              str(snap.hit_at(p["content_x"], p["content_y"])))
        check("the gap ring belongs to nothing", snap.hit_at(0, 0) is None,
              str(snap.hit_at(0, 0)))
