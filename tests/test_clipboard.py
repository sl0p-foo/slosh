...
LINE = ('src/app.c:1234:5: error in foo_bar(x); see https://a.b/c?d=1 '
        'or "quoted" \u65e5\u672c\u8a9e\u3067\u3059 end')
    ...
    with Session(a_line(), cols=110, rows=12) as s:
        ...
        for label, needle, offset, want in (
                ...
                ("quotes separate, so the word inside them is the word",
                 "quoted", 2, "quoted"),
                ("a run of wide glyphs is one word", "\u65e5\u672c\u8a9e", 1,
                 "\u65e5\u672c\u8a9e\u3067\u3059")):
...
def test_the_highlight_is_the_copy():
    ...
    with Session(a_line(), cols=110, rows=12) as s:
        ...
        if runs:
            p = s.pane()
            check("on the row that was clicked", runs[0]["y"] == p["content_y"],
                  str(runs[0]))
            check("as wide as the word that was copied", runs[0]["w"] == len(clip),
                  "%s vs %r" % (runs[0], clip))
...
def test_a_click_that_finds_no_word_leaves_the_clipboard_alone():
    ...
    with Session(a_line(), cols=110, rows=12) as s:
        ...
        check("double-clicking a separator copies nothing new",
              on_separator == first, repr(on_separator))

        on_blank = worded(s, " end")
        check("nor does double-clicking a blank cell", on_blank == first,
              repr(on_blank))
...
def test_two_clicks_on_different_words_are_not_a_double_click():
    ...
    with Session(a_line(), cols=110, rows=12) as s:
        ...
        s.settle(80)
        check("neither click selected anything on its own",
              s.api("clipboard")["text"] in (None, ""), repr(s.api("clipboard")["text"]))
        dbl_click(s, x2, row, 1)      # ...and the second cell's pair completes
        s.settle(80)
        check("the pair that lands on one cell selects that word",
              s.api("clipboard")["text"] == "quoted", repr(s.api("clipboard")["text"]))
...
def test_slow_clicks_are_two_clicks():
    with Session(a_line(), cols=110, rows=12) as s:
        ...
        dbl_click(s, x, p["content_y"], 1)
        s.settle(80)
        check("two slow clicks select no word",
              s.api("clipboard")["text"] in (None, ""), repr(s.api("clipboard")["text"]))
...
def test_the_separators_are_configurable():
    ...
    with Session(a_line(), cols=110, rows=12, config=f.name) as s:
        ...
        check("with no separators, punctuation is part of the word",
              got == "foo_bar(x);", repr(got))
    ...
    with Session(a_line(), cols=110, rows=12, config=g.name) as s:
        ...
        check("and a listed character splits what it is listed in", got == "foo",
              repr(got))
    ...
    with Session(a_line(text), cols=80, rows=10, config=h.name) as s:
        ...
        check("a quote can be a separator, escaped through the config",
              got == "hi", repr(got))
...
def test_a_word_in_scrollback_is_the_word_on_screen():
    ...
    they stay the same."""
    body = "; ".join("printf 'line%02d unique_%02d\\r\\n'" % (i, i) for i in range(1, 41))
    with Session(echoing(body), cols=60, rows=12) as s:
        ...
        check("and the word copied is the one on screen", got == word,
...
def test_a_mouse_program_keeps_its_double_clicks():
    ...
    with Session(tracking, cols=60, rows=10) as s:
        ...
        dbl_click(s, col, row)
        s.settle(80)
        check("a plain double-click is the program's", s.api("clipboard")["text"] in (None, ""),
        ...
        for _ in range(2):                      # the same pair with shift held
            ...
            s.send(rf"\e[<4;{col + 1};{row + 1}m")
        s.settle(80)
        check("shift takes it back and selects the word",
