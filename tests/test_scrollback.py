...
def spew(n):
    ...
    return [
        "/bin/sh",
        "-c",
        'i=1; while [ $i -le %d ]; do echo "line-$i"; i=$((i+1)); done; read x' % n,
    ]
...
def test_a_pane_keeps_the_configured_number_of_lines():
    ...
    with Session(spew(3000), cols=80, rows=8, config=cfg) as s:
        n = kept(s, 3000)
        check(
            "a small setting is honoured",
            n is not None and 250 < n < 700,
            "kept %s of 3000" % n,
        )
    os.unlink(cfg)
    ...
    with Session(spew(3000), cols=80, rows=8, config=cfg) as s:
        n = kept(s, 3000)
        check(
            "and a larger one keeps more",
            n is not None and n > 1500,
            "kept %s of 3000" % n,
        )
    os.unlink(cfg)
...
def test_zero_keeps_no_history_at_all():
    ...
    with Session(spew(3000), cols=80, rows=8, config=cfg) as s:
        n = kept(s, 3000)
        check(
            "nothing is retained past the viewport",
            n is not None and n <= 10,
            "kept %s of 3000" % n,
        )
        check(
            "and scrolling up says so",
            "scrolled" not in s.snapshot().screen(),
            s.snapshot().screen(),
        )
    os.unlink(cfg)
...
def test_the_default_is_not_a_library_accident():
    ...
    with Session(spew(3000), cols=80, rows=8) as s:
        n = kept(s, 3000)
        check(
            "the default keeps everything a 3000-line run printed",
            n is not None and n > 2900,
            "kept %s of 3000" % n,
        )
...
def test_the_byte_ceiling_is_the_other_limit():
    ...
    with Session(spew(3000), cols=80, rows=8, config=cfg) as s:
        n = kept(s, 3000)
        check(
            "a low ceiling clips a high line count",
            n is not None and n < 1200,
            "kept %s of 3000" % n,
        )
    os.unlink(cfg)
    ...
    with Session(spew(3000), cols=80, rows=8, config=cfg) as s:
        n = kept(s, 3000)
        check(
            "and no ceiling lets the line count stand",
            n is not None and n > 2900,
            "kept %s of 3000" % n,
        )
    os.unlink(cfg)
...
def test_it_applies_to_the_next_pane_not_the_ones_holding_history():
    ...
    with Session(spew(3000), cols=80, rows=8, config=cfg) as s:
        before = kept(s, 3000)
        check(
            "the pane has its history",
            before is not None and before > 1500,
            "kept %s" % before,
        )
        ...
        after = kept(s, 3000)
        check(
            "and the pane already open keeps what it had",
            after is not None and after > 1500,
            "kept %s" % after,
        )
    os.unlink(cfg)
