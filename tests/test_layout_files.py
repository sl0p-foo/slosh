...
def test_structure():
    with session_with(LAYOUT, cols=78, rows=18) as s:
        ...
        tabs = s.tabs()
        check(
            "a layout builds its tabs",
            [t["name"] for t in tabs] == ["api", "notes"],
            str(tabs),
        )
        check(
            "and only its tabs (the default one is replaced)", len(tabs) == 2, str(tabs)
        )
        check(
            "tab purposes are declared, so nothing can override them",
        ...
        check(
            "pane purposes come from the layout",
            purposes == ["agent:main", "service:web", "logs:tail"],
            str(purposes),
        )
        check(
            "pane purposes are declared too",
            all(p["purpose_declared"] for p in panes if p["purpose"]),
            str(panes),
        )

        first, web, logs = panes[0], panes[1], panes[2]
        check(
            "the tab body splits into columns by default",
            first["x"] < web["x"] and first["y"] == web["y"],
            f"{first['x']},{first['y']} vs {web['x']},{web['y']}",
        )
        check(
            "a nested pane with split=rows stacks its children",
...
def test_suspended():
    with session_with(LAYOUT, cols=78, rows=18) as s:
        ...
        snap = s.snapshot()
        check(
            "a suspended pane says what it would run",
            "press a key to run:" in snap.screen(),
            repr(snap.screen()[:400]),
        )
        check(
            "but has not run it",
            "web-running" not in snap.pane_text(web),
        ...
        check("a keystroke starts it", now and not now[0]["suspended"], str(s.panes()))
        check(
            "and the command actually runs",
            "web-running" in s.snapshot().pane_text(now[0]),
            repr(s.snapshot().pane_text(now[0])),
        )
        check(
            "the keystroke that started it is not also sent to it",
...
def test_declared_purposes_are_locked():
    with session_with(LAYOUT, cols=78, rows=18) as s:
        ...
        r = s.api(
            ...
            target="pane",
            id=agent["id"],
            purpose="evil:relabelled",
            declared=False,
        )
        check(
            "a layout-declared pane purpose cannot be taken in-band",
            not r["ok"],
            str(r),
        )
        check(
...
def test_cwd():
    ...
    with session_with(text, cols=70, rows=14) as s:
        ...
        snap = s.snapshot()
        check(
            "a tab's cwd is inherited by its panes",
            "/tmp" in snap.pane_text(panes[0]),
            repr(snap.pane_text(panes[0])),
        )
        check(
            "a pane can override it",
            "/usr" in snap.pane_text(panes[1]),
...
def test_apply_over_the_api():
    ...
    with Session(SH, cols=70, rows=14) as s:
        ...
        frag = (
            'layout { tab name="added" purpose="project:x.deadbeef" '
            '{ pane purpose="agent:main" } }'
        )
        r = s.api("apply-layout", kdl=frag)
        s.settle()
        check(
            "apply-layout answers with the tab list",
            r["ok"] and "tabs" in r,
            str(r)[:100],
        )
        check(
            "the tab is added, not replacing what was there",
            len(s.tabs()) == 2 and s.tabs()[1]["name"] == "added",
            str(s.tabs()),
        )
        check(
            "its purpose is declared",
            s.tabs()[1]["purpose"] == "project:x.deadbeef"
            and s.tabs()[1]["purpose_declared"],
            str(s.tabs()[1]),
        )
        ...
        check(
            "a caller that diffs first does not duplicate",
            len(s.tabs()) == 2,
            str(s.tabs()),
        )
        ...
        r = s.api(
            "apply-layout", kdl='layout { tab name="only" { pane } }', replace=True
        )
        s.settle()
        check(
            "replace=true rebuilds the session",
            len(s.tabs()) == 1 and s.tabs()[0]["name"] == "only",
            str(s.tabs()),
        )
...
def test_bad_layouts():
    with Session(SH, cols=70, rows=14) as s:
        ...
        check(
            ...
            s.api("alive")["alive"] and len(s.tabs()) == 1,
            str(s.tabs()),
        )
    ...
    with Session(SH, cols=70, rows=14, layout=broken) as s:
        s.settle()
        check(
            "a broken layout at startup still gives you a session",
            s.api("alive")["alive"] and len(s.panes()) == 1,
            str(s.panes()),
        )
    os.unlink(broken)
