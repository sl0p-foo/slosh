...
def test_wheel_scrolls():
    with Session(SH, cols=50, rows=12) as s:
        ...
        fill(s)
        p = s.pane()
        check("the viewport is at the bottom", "60" in s.snapshot().pane_text(p),
        ...
        s.settle(80)
        check("and forward again", "60" in s.snapshot().pane_text(p),
              repr(s.snapshot().pane_text(p)))
...
def test_indicator():
    with Session(SH, cols=50, rows=12) as s:
        ...
        p = s.pane()
        snap = s.snapshot()
        check("no indicator when the viewport is live",
        ...
        x = row.index("▲")
        check("the indicator is clickable",
              snap.hit_at(x, p["y"]) == f"scrollbottom:{p['id']}",
              str(snap.hit_at(x, p["y"])))
        s.click(x, p["y"])
        s.settle(80)
        snap = s.snapshot()
        check("clicking it returns to the live view",
...
def test_keyboard():
    with Session(SH, cols=50, rows=12) as s:
        ...
        s.settle(80)
        check("C-a PageUp scrolls a page", "60" not in s.snapshot().pane_text(p),
              repr(s.snapshot().pane_text(p)))
        ...
        check("C-a Home goes to the top of the scrollback",
              "sh-5.3$" in text and "60" not in text, repr(text[:120]))

        s.send(r"\x01\e[F")  # C-a End
        s.settle(80)
        check("C-a End returns to the bottom",
...
def test_typing_returns_to_the_live_view():
    with Session(SH, cols=50, rows=12) as s:
        ...
        check("and the indicator goes with it",
...
def test_scroll_position_is_per_pane():
    with Session(SH, cols=90, rows=14) as s:
        ...
        check("the other is not", "▲" not in frame_of(right),
              repr(frame_of(right)))
...
def test_who_owns_the_wheel():
    ...
    alt = ["/bin/sh", "-c",
           'stty raw -echo; printf "\\033[?1049h"; cat -v']
    with Session(alt, cols=60, rows=12) as s:
        ...
        check("on the alternate screen the wheel becomes arrow keys",
              out.count("^[[A") == 3, repr(out[:120]))
        check("and does not scroll us instead",
              "▲" not in s.snapshot().line(p["y"]), repr(s.snapshot().line(p["y"])))
    ...
    with Session(tracker, cols=60, rows=12) as s:
        ...
        s.settle(80)
        out = s.snapshot().pane_text(p)
        check("a mouse-tracking program gets the wheel", "^[[<64;3;3M" in out,
              repr(out[:120]))
...
def test_output_while_scrolled():
    with Session(SH, cols=50, rows=12) as s:
        ...
        s.settle(150)
        after = s.snapshot()
        check("new output does not yank the viewport back",
              after.pane_line(p, 0) == before,
              f"{before!r} -> {after.pane_line(p, 0)!r}")
        check("but the indicator shows the gap growing",
              "▲" in after.line(p["y"]), repr(after.line(p["y"])))
